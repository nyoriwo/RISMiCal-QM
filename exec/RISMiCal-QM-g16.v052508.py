...
[NEW] Enhanced FC mode output to dynamically extract and display TD-DFT excited states, 
      target state E(UU), solute-solvent interaction (SE_ES), and effective charges.
...
# [CRITICAL FIX] Safe E(UU) Extraction: Prevent orientation mismatch and checkpoint contamination
def get_euu_gaussian(base_name, params, chk_file, udata, qm_indices):
    """
    Evaluates the pure internal energy E(UU) of the polarized density 
    by running a 1-step energy calculation without external point charges.
    ...
    """
    euu_gjf = f"{base_name}_euu.gjf"
    euu_gout = f"{base_name}_euu.gout"
    euu_chk = f"{base_name}_euu.chk"
    
    # Safely copy the main checkpoint to avoid altering it during the E(UU) extraction
    if os.path.exists(chk_file):
        shutil.copy(chk_file, euu_chk)
    
    qmopt_raw = params.get('qmopt', '')
    raw_lines = qmopt_raw.split('\\')
    link0_lines, route_line, title_line, chg_mult = [], "", "Internal Energy Extraction", "0 1"
    ...
    link0_lines.append(f"%chk={euu_chk}")
    
    # Remove ONLY the external charge keyword. We MUST KEEP NoSymm (if present) 
    # to ensure the orientation exactly matches the checkpoint MOs.
    route_line = re.sub(r'(?i)\bcharge\b', '', route_line)
    ...
    subprocess.run([G16_CMD, euu_gjf, euu_gout])
    
    # [MODIFIED] Removed termination check here because SCF=(MaxCycle=1) 
    # intentionally does not reach full convergence, causing a false warning.
        
    # Read the extracted energy using the unified robust reader
    e_uu_hartree = read_target_energy(euu_gout)
    return e_uu_hartree * HARTREE_TO_KJMOL
...
def main():
    ...
    if fc_mode:
        ...
        # Extract pure internal E(UU) and Solute-Solvent SE_ES for the target state
        e1_hartree = read_target_energy(gout_file)
        e_uu_kjmol = get_euu_gaussian(base_name, params, chk_file, udata, qm_indices)
        e_uu_hartree = e_uu_kjmol / HARTREE_TO_KJMOL
        
        # Calculate interaction energy directly based on E1 - E_UU
        se_es_kjmol = (e1_hartree - e_uu_hartree) * HARTREE_TO_KJMOL
        
        print("\n  [Target State (Root) Properties]")
        print(f"  QM Internal Energy E(UU) : {e_uu_kjmol:.4f} kJ/mol")
        print(f"  Solute-Solvent SE_ES     : {se_es_kjmol:.4f} kJ/mol")
    ...
    while True:
        ...
        # Extract pure internal E(UU) free of qvcore artifacts safely without orientation mismatch
        e_uu_kjmol = get_euu_gaussian(base_name, params, chk_file, udata, qm_indices)
        ...
        # Calculate Total Free Energy properly using pure E(UU) and RISM SFE
        e_tot = e_uu_kjmol + e_mv + sfe
        
        print(f"  E(UU) Internal  = {e_uu_kjmol:.4f} kJ/mol")
        if mm_indices: print(f"  E_MV            = {e_mv:.4f} kJ/mol")
        print(f"  SE_ES           = {se:.4f} kJ/mol")
        print(f"  SFE_SC          = {sfe:.4f} kJ/mol")
        print(f"  E_TOTAL         = {e_tot:.4f} kJ/mol")
    ...
    print("="*55)
    print(" >>> SCF Converged! <<<")
    print(f" Gas-phase Energy (E_gas)    : {e_gas:15.5f} kJ/mol")
    if mm_indices:
        print(f" QMMM Gas Energy (E_QMMM_gas): {e_qmmm_gas:15.5f} kJ/mol")
        print(f" QM-MM Interaction (gas)     : {e_qmmm_int:15.5f} kJ/mol")
    print(f" Total Free Energy (E_TOTAL) : {e_tot:15.5f} kJ/mol")
    print("-" * 55)
    print(f" QM Internal Energy (E_UU)   : {e_uu_kjmol:15.5f} kJ/mol")
    if mm_indices:
        print(f" MM-Solv Int. (E_MV)         : {e_mv:15.5f} kJ/mol")
    print(f" Electrostatic Int. (SE_ES)  : {se:15.5f} kJ/mol")
