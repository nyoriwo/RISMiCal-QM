...
"""
RISMiCal-QM.py
A wrapper script for performing QM/3D-RISM (KSDFT/3D-RISM, 3D-RISM-SCF, QM/MM/3D-RISM) calculations using RISMiCal and Gaussian 16.
...
# Write external charge by MM atom into qv file
...
# Read RESP charge from Gaussian output
...
# [MODIFIED] Added 'params' to arguments to fix scope issue for qmopt parsing
# Run cubegen to get cub file
...
# Add Electrostatic potential from MM atom to cube
...
# Update RISMiCal input (Coordinate and charge of QM atom)
...
# Read Gaussian output file to get SCF energy
...
def main():
    ...
    if len(sys.argv) < 2: 
        print("Usage: python RISMiCal-QM.py <input_file> [-FC]"); sys.exit(1)
    ...
    print(f"--- RISMiCal-QM Started ({base_name}) ---")
    ...
    if fc_mode:
        ...
        # iter_num=1 ensures that the 'Charge' keyword is added and qv_file is read
        ...
        if not check_gaussian_termination(gout_file):
            print(f"  [ERROR] Gaussian 16 terminated abnormally. Check {gout_file}")
            sys.exit(1)
            
        # Copy to a specific FC output file for clarity
        shutil.copy(gout_file, f"{base_name}.gout.fc")
        print("  >>> FC Calculation Completed Successfully! <<<")
        print(f"  Check '{base_name}.gout.fc' for excited state properties.")
    ...
    run_cubegen(fchk_file, ascii_cube, ngrid3d, rdelta3d, params) # [MODIFIED] Added params
